...
impl MetricsCollector {
    ...
    pub fn record_events_dropped(&self, count: u64) {
        if count > 0 {
            ...
            if count > 100 {
                error!("High number of dropped events: {}, potential system issue", count);
                self.record_error();
            }
        }
    }
    ...
    pub fn get_metrics(&self) -> Metrics {
        ...
        debug!("Metrics collected - processed: {}, dropped: {}, errors: {}, uptime: {}s", 
               events_processed, events_dropped, errors_total, uptime.as_secs());
    ...
    async fn collect_metrics_safely(&self) -> Result<Metrics, String> {
        ...
        let uptime = self.start_time.elapsed();
        if uptime.as_secs() > 86400 { // 24 hours
            return Err("Metrics collection running too long, potential memory leak".to_string());
        }
        
        // Check error rate threshold
        let error_rate = self.errors_total.load(Ordering::Relaxed) as f64 / uptime.as_secs_f64().max(1.0);
            ...
            loop {
                ...
                match metrics_collector.collect_metrics_safely().await {
                    Ok(metrics) => {
                        ...
                        debug!("Monitoring config: {}", metrics_collector.get_monitoring_config());
                    }
                    Err(e) => {
                        consecutive_failures += 1;
                        error!("Metrics collection failed (attempt {}/{}): {}", consecutive_failures, max_failures, e);
                ...
                match metrics_collector.check_health_safely().await {
                    Ok(health_status) => {
                        ...
                        if !health_status.is_healthy {
                            warn!("Health check failed at {}: {}", 
                                  health_status.timestamp.format("%Y-%m-%d %H:%M:%S UTC"), 
                                  health_status.message);
                            
                            // Handle unhealthy state
                            metrics_collector.handle_unhealthy_state(&health_status).await;
                        } else {
                            debug!("Health check passed at {}: {}", 
                    ...
                    Err(e) => {
                        ...
                        error!("Health check error (attempt {}/{}): {}", consecutive_health_failures, max_health_failures, e);
    ...
    async fn check_health(&self) -> HealthStatus {
        ...
        let message = if final_health {
            format!("System healthy, uptime: {}s, error rate: {:.2}%", 
                   uptime.as_secs(), error_rate * 100.0)
        } else {
            format!("System unhealthy, uptime: {}s, error rate: {:.2}%, memory: {}", 
                   uptime.as_secs(), error_rate * 100.0, if memory_ok { "OK" } else { "WARNING" })
        };
    ...
    async fn check_health_safely(&self) -> Result<HealthStatus, String> {
        ...
        if error_rate > 0.8 { // 80% error rate
            return Err(format!("Error rate too high for health checks: {:.2}%", error_rate * 100.0));
        }
    ...
    async fn handle_unhealthy_state(&self, health_status: &HealthStatus) {
        ...
        error!("System health degraded at {}: {}", 
               health_status.timestamp.format("%Y-%m-%d %H:%M:%S UTC"), 
               health_status.message);
